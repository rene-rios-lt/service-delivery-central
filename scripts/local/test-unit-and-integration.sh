...
render() {
  ...
  pln "  ${C_DIM}Frontend E2E (Playwright/Appium) needs a live system — run test-e2e.sh (or the full test-all.sh).${C_RESET}"
}
...
# When invoked by the full-suite orchestrator (test-all.sh), emit this run's aggregated rows so it
# can fold them into one unified table alongside the live-system E2E results. cell_stats echoes
# "total passed failed state"; tr turns it into the pipe-delimited row sd_render_results_table wants.
if [ -n "${SD_SUMMARY_FILE:-}" ]; then
  {
    printf 'Backend|Unit|%s\n'        "$(cell_stats be_domain be_app be_arch | tr ' ' '|')"
    printf 'Backend|Integration|%s\n' "$(cell_stats be_infra be_api | tr ' ' '|')"
    printf 'Frontend|Unit|%s\n'       "$(cell_stats fe_client | tr ' ' '|')"
    printf 'Simulator|Unit|%s\n'      "$(cell_stats sim | tr ' ' '|')"
  } >> "$SD_SUMMARY_FILE"
fi
