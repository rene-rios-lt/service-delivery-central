...
| [**BUG-038**](bug.md) | Frontend | **Bug** — Rep idle screen raises Blazor's "An unhandled error has occurred" banner when the RepHub SignalR connection can't be established at mount: `RepIdle.OnInitializedAsync` awaits an unguarded `StartAsync`, and `.WithAutomaticReconnect()` does not cover the initial connect, so an unreachable RepHub throws uncaught. Found via live iOS-simulator debugging when an offer's countdown expired into the idle screen while the backend was unreachable (also triggers on backend restart / network blip / macOS sleep dropping the socket). Fix: catch + retry the initial connect, surface connection state for a non-fatal "reconnecting" indicator, and never let a connect exception escape `OnInitializedAsync`. |
