...
Plus **9 engineering-quality stories** ([`quality.md`](quality.md)) — `QUAL-001` – `QUAL-009`. `QUAL-001`–`QUAL-004` are complete; `QUAL-005`–`QUAL-008` are open, filed from a retrospective on why integration defects clustered in the frontend (many untestable boundaries + a live net that arrived late) while the backend and simulator stayed clean (one peer, `smoke.sh` from day one). They harden the frontend integration loop: run the live net early (`QUAL-005`), enforce wire-contract integrity across repos (`QUAL-006`), test the real composition root not isolated components (`QUAL-007`), and treat each client runtime as its own integration target (`QUAL-008`). `QUAL-009` (open) live-verifies the FE-023/BE-028/SIM-009 go-off-duty / heartbeat-timeout chain end-to-end, since each layer was proven only in isolation.
