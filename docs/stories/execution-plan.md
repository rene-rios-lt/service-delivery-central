...
| [**BUG-035**](bug.md) | Backend | **Bug** — `AvailableVehicleDto` carries no vehicle model, so take-over rows and the idle card show "V-001" not "IA-4471 · Transit 350" (per the mockups). Deferred from BUG-034 (frontend-only). Fix via `/master`: add a model field to the entity/seed + DTO + frontend `IdleVehicle`/`ClaimedVehicle`. |
...
Plus **35 bugs** ([`bug.md`](bug.md)) — `BUG-001` – `BUG-035`; all resolved except `BUG-032` (Appium job-offer test precondition) and `BUG-035` (vehicle model field) — both follow-ups. `BUG-003`–`BUG-015` were central-repo doc/pipeline fixes (shipped via `/ship-it`). `BUG-024`–`BUG-026` were frontend E2E failures found via `test-e2e.sh`. `BUG-027`–`BUG-031` were found via `test-appium.sh` (backend idle-vehicle semantics, frontend REST + SignalR auth headers, startup error, and Appium harness defects); the Appium suite now passes 4/9, with the remaining 5 tracked in `BUG-032`. `BUG-033` (rep take-over/idle mockup fidelity, PR #36) and `BUG-034` (idle view showed a hardcoded vehicle, PR #37) were fixed via `/master`; `BUG-035` (surface the vehicle model in the DTO) is the open follow-up.
