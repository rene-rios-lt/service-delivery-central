...
| [**BUG-034**](bug.md) | Frontend | **Bug** — Idle view card **and** app-bar subtitle show a hardcoded vehicle (IA-4471) instead of the one the rep took over (e.g. V-001): the take-over flow never hands the selected vehicle to the idle view (RepIdleViewModel depends on a stubbed `ClaimedVehicle`). Found via the Appium take-over flow. Fix via `/master`: carry the selection forward, drive card + subtitle from it, surface the model. |
...
Plus **34 bugs** ([`bug.md`](bug.md)) — `BUG-001` – `BUG-034`; all resolved except `BUG-032` (Appium job-offer test precondition) and `BUG-034` (real claimed-vehicle hand-off) — both follow-ups. `BUG-003`–`BUG-015` were central-repo doc/pipeline fixes (shipped via `/ship-it`). `BUG-024`–`BUG-026` were frontend E2E failures found via `test-e2e.sh`. `BUG-027`–`BUG-031` were found via `test-appium.sh` (backend idle-vehicle semantics, frontend REST + SignalR auth headers, startup error, and Appium harness defects); the Appium suite now passes 4/9, with the remaining 5 tracked in `BUG-032`. `BUG-033` (rep take-over/idle mockup fidelity) was fixed via `/master` (PR #36); `BUG-034` (idle view shows a hardcoded vehicle, not the one taken over) is the open follow-up.
