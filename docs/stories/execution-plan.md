...
| [**BUG-018**](bug.md) | Simulator | **Bug** — automated job stalls at `Within15Miles`: the truck stops navigating ~1–2 km short of the requester, never hits the arrival threshold, so no auto-arrive/dwell/complete. Found by the third headless run. Fix: keep navigating through `Within15Miles` until reached (`VehicleDriveResolver`). |
