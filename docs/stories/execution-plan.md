...
| [**BUG-045**](bug.md) | Backend / Simulator | **Bug** — Concurrent take-overs collide on the first-listed available vehicle (V-001), so a second assignable rep can't be acquired cleanly on a clean live start — a distinct concurrency/ergonomics facet of the fixed BUG-027 (cross-referenced, not contradicted). Found during FE-018 redirect E2E setup (worked around by claiming distinct vehicles V-002/V-003). Needs backend triage: server-side spread/reserve or a documented client retry-on-409. |
