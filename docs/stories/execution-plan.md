...
| [**BUG-055**](bug.md) | Frontend | **Bug** — Playwright `RequesterRedirectTests` arrange helper `WaitForEnRouteRepAtAsync` matches only `RepState == "EnRoute"`, so a rep already advanced to `Within15Miles` is missed and the redirect setup starves for 30 s and throws "never assigned" — even though the request is matched and reps are driving to it (zero expired offers). A test-only state-window race, surfaced *more* by BUG-053's fix (healthy hubs → instant accept → shorter `EnRoute` window). Found during BUG-053's completed live gate (2026-07-21, `test-playwright.sh` cycle 3). Fix via `/master`: broaden the predicate to `EnRoute` **or** `Within15Miles`. |
