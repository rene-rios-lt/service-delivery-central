...
| ~~[**BUG-052**](bug.md)~~ | ~~Simulator~~ | ~~**Bug** — Simulator rep claim loop livelocks: every vehicle-less rep picks `FirstOrDefault` of `/vehicles/available` (no ordering/jitter/per-rep offset), but that listing includes claimed-but-idle vehicles (take-over semantics, BUG-027/FE-007), so reps stampede the front-of-list vehicle, 409 forever with no skip/backoff, and vehicles 5–8 can get **zero** claim attempts for a whole run (observed: veh1 464 attempts, veh4–8 0; isolation: only veh1 ever claimed). Non-deterministic fleet starvation — a strong candidate for the true root cause behind the BUG-049 family. Found during BUG-049's live gate (its fleet diagnostic exposed it). Fix via `/master`: collision-free/self-healing simulator claim selection (distinct targets + advance-past-409 with jitter); backend take-over listing unchanged.~~ |
| ~~[**BUG-053**](bug.md)~~ | ~~Simulator~~ | ~~**Bug** — Simulator rep RepHub connections die silently and stay dead: no logger on the `HubConnectionBuilder`, `Closed` unobserved, bounded reconnect then permanent silent death, no post-startup connection reconcile — so deaf-but-`Available` reps let every job offer burn its full 60 s TTL (5 of 8 reps deaf in one observed boot; a fresh-listener probe received the exact offers the sim missed, exonerating the backend send path). The true root cause of the remaining BUG-049 pending-starvation flake post-BUG-052. Found during BUG-049's resumed live gate. Fix via `/master`: hub lifecycle logging (QUAL-012's sim analogue) + self-healing reconnect (Closed→restart with backoff; reconcile `HubConnection.State` each FleetReconciler tick).~~ |
| ~~[**BUG-054**](bug.md)~~ | ~~Backend~~ | ~~**Bug** — Expired job offers permanently poison a request's skip list: `GetSkippedRepIdsForRequestAsync` treats `Expired` ≡ `Declined`, never cleared, so a request whose qualified pool cycles once (e.g. via BUG-053's deaf reps) is permanently unmatchable — observed live: two requests starved forever while 4+ qualified reps sat `Available`. Related observations recorded in the entry: accept-while-on-active-job unguarded (one rep held 3 concurrent jobs) and sweeper/reconciler race creating duplicate concurrent offers. Fix via `/master` after a product decision on expiry semantics (only `Declined` skips durably; expired reps become re-offerable).~~ |
| [**BUG-055**](bug.md) | Frontend | **Bug** — Playwright `RequesterRedirectTests` arrange helper matches only `RepState == "EnRoute"`, so a rep already advanced to `Within15Miles` is missed and the redirect setup starves for 30 s and throws "never assigned" — even though the request is matched and reps are driving to it (zero expired offers). A test-only state-window race, surfaced *more* by BUG-053's fix (healthy hubs → instant accept → shorter `EnRoute` window). Found during BUG-053's completed live gate (2026-07-21). The *completion*-path sibling of this exact helper race was fixed + the helper renamed to `WaitForRepServingRequestAtAsync` in frontend PR #78 (BUG-054 gate); the redirect path stays open. Fix via `/master`: **corrected** — broadening the accept-set is UNSAFE for redirect (`/dispatcher/redirect` is EnRoute-only and `Within15Miles` is a one-way latch); keep the rep `EnRoute` until redirect (frontend), or make the proximity transition bidirectional (backend product decision). See the bug entry. |
| [**BUG-056**](bug.md) | Frontend | **Bug** — Playwright `DispatcherFleetMapTests.…WhenFleetMarkerClicked` clicks a stale `IElementHandle` from `WaitForSelectorAsync`; the 3 s Maps-SDK marker re-render detaches it → `Element is not attached to the DOM`. Same DOM-churn class as BUG-046 (Appium). Found during BUG-053's completed live gate (2026-07-21, `test-playwright.sh` cycle 1). Fix via `/master`: use an auto-retrying `Locator` that re-resolves the marker on each click attempt. |
| ~~[**BUG-057**](bug.md)~~ | ~~Backend~~ | ~~**Bug** — `POST /job-offers/{id}/accept` has no active-job guard: `AcceptJobOfferCommandHandler` calls `GoEnRoute` (which overwrites `ActiveRequestId`) without checking `repState.IsOnActiveJob()` or that the offer is still `Pending`, and nothing serializes concurrent accepts — so an already-servicing rep can accept another offer and one rep can hold multiple concurrent jobs (observed: 3 at once). BUG-054 related observation (a), filed 2026-07-21 from BUG-049's resumed gate. Fix via `/master`: reject accept when on an active job / offer not Pending + optimistic concurrency.~~ |
| ~~[**BUG-058**](bug.md)~~ | ~~Backend~~ | ~~**Bug** — Expiry sweeper and pending-request reconciler race in `MatchingService.RunAsync`, which creates a `Pending` offer with no idempotency check — so `ExpiredJobOfferSweeper` and `PendingRequestReconciler` can each offer the same nearest rep, yielding duplicate concurrent offers for one request+rep (observed: Rep 8 ×2, Rep 7 ×2). BUG-054 related observation (b), filed 2026-07-21 from BUG-049's resumed gate. Fix via `/master`: dedup live offers / uniqueness on `(ServiceRequestId, Pending)` / serialize the two passes; preserve BUG-054 re-offer semantics.~~ |
| ~~[**BUG-059**](bug.md)~~ | ~~Backend~~ | ~~**Bug** — Rep proximity transition is a one-way `EnRoute → Within15Miles` latch (`UpdateVehiclePositionCommandHandler` recomputes proximity only while already `EnRoute`), so a rep that later moves back out past 15 mi never returns to `EnRoute` and — since `/dispatcher/redirect` is `EnRoute`-only — can never be redirected again; combined with the simulator's Navigate driver this made BUG-055's redirect arrange impossible frontend-only (proven across 3 live gates, 2026-07-22). Fix via `/master` (backend PR #58): bidirectional `EnRoute ⇄ Within15Miles` recompute with **hysteresis** (enter < 15 mi / exit ≥ 17 mi, dead-band 15–17 mi prevents flapping) — product decision recorded in ADR-0012. Unblocks BUG-055.~~ |
| ~~[**BUG-061**](bug.md)~~ | ~~Backend~~ | ~~**Bug** — A job offer whose rep then accepts a different job (second accept 409-rejected by the BUG-057 guard) is left `Pending` until ~60 s expiry instead of being released and re-matched, so a contended request stalls. Surfaced *because* BUG-057's guard now works (before, the second accept clobbered). Found during BUG-055's close-out live gate (2026-07-23): rep4/rep8 each got two Gold offers, accepted one, 409 on the other, and the losing request never re-matched inside the 45 s window. Fix via `/master`: release-and-re-match the rejected offer immediately (backend, preferred) and/or sim decline-on-409. **Last blocker for BUG-055's redirect test** (059/060/057/058 + the arrange fix all landed).~~ |
| ~~[**BUG-060**](bug.md)~~ | ~~Simulator~~ | ~~**Bug** — Simulator types `JobOfferReceived.etaMinutes` as `int` while the backend sends a `double` (unrounded Haversine ETA); `System.Text.Json` throws on any fractional value, the whole hub invocation fails to bind, and the offer is **silently dropped** — the rep never accepts (only whole-number-ETA offers, e.g. a rep exactly at the requester, bind). ADR-0011 wire-contract drift, sibling of BUG-036. Found during BUG-055's live gate (2026-07-22/23): the redirect test's post-redirect displaced request was never re-accepted. Fix via `/master`: type `etaMinutes` as `double` (frontend already does) + captured-payload round-trip test with a fractional ETA. Unblocks BUG-055's live gate.~~ |
...
| [QUAL-014](quality.md) | Frontend | **Reliability defect** — self-healing SignalR reconnect in all three hub clients: today they build `.WithAutomaticReconnect()` bare (4 retries ~42 s → `Closed` permanently), never observe `Closed`/`Reconnecting`, and only the cold-connect back-off (BUG-038) exists — a post-establishment drop kills the connection silently forever. The frontend twin of the BUG-053 "deaf-but-available" mechanism. Add an unbounded retry policy + `Closed`→restart, re-establish subscriptions on reconnect, surface state. Ships via `/master`. (Filed 2026-07-21 from the shared-code design investigation; ordered #1 = highest leverage.) |
| [QUAL-015](quality.md) | Frontend | **Leak defect** — `JobOffer.razor` registers `OnJobOfferExpired` via `_connection.On(...)` but discards the returned `IDisposable` and never unsubscribes in `Dispose`; because `RepHubService` is session-scoped, every visit to `/rep/offer` stacks another handler capturing a disposed component. Return + dispose the subscription; prove one live handler after N visits. Ships via `/master`. |
| [QUAL-016](quality.md) | Frontend | **Contract integrity** — enforce ADR-0011 throw-on-unmapped tier across ALL wire types via one `ServiceTierWire.Parse`: only `JobOfferReceivedWirePayload` parses-or-throws today; `RedirectPayload`/`ActiveJobContext`/`DispatcherFleetEntryDto` keep tier as a raw string that's never parsed (so it can never throw) and flows into `sd-badge--{tier}` — the BUG-036 invisible-badge class. Add real boundary mappers + captured-payload tests. Ships via `/master`. |
| [QUAL-017](quality.md) | Frontend | **High-leverage dedup (composition)** — extract a `ResilientHubConnection` *collaborator* (injected, not a base class) to kill the ~80% copy-paste across the three hub clients (back-off, `BuildConnection`, token provider, `StartAsync`/`RetryConnect`/`StopAsync`/`DisposeAsync`) and unify the `StartAsync` idempotency guard that Requester has but Rep/VehiclePosition lack; removes the test-seam-constructor smell (inject a fake instead). Vehicle for QUAL-014/015. Ships via `/master`. |
| [QUAL-018](quality.md) | Frontend | **High-volume dedup** — `INotifyStateChanged` *interface* on the VM side (not an `ObservableViewModel` base) + an `ObservingComponentBase<TViewModel>` on the UI side (a base class is idiomatic here — Blazor is already a hierarchy) to replace the 5 hand-rolled `StateChanged` events and the per-page subscribe/marshal/unsubscribe triad, structurally eliminating the QUAL-015 leak class. Ships via `/master`. |
| [QUAL-019](quality.md) | Frontend | **Consistency + defect** — unify the ~12 `Http*Service` response→result mappings behind one light helper (3 competing styles today; `409→Conflict` copy-pasted 3×) and fix the empty-2xx-body `NullReferenceException` force-unwraps in `HttpAuthService`/`HttpServiceRequestService`. Extension/helper, not a heavy base class. Ships via `/master`. |
| [QUAL-020](quality.md) | Frontend | **Contract drift risk** — one `RepStates` source of truth for the rep-state wire strings (`"EnRoute"`/`"Within15Miles"`/`"OnSite"`), currently declared twice (`ActiveJobViewModel`, `RequesterTrackingViewModel`) with "must match backend" comments; add a drift-guard test. Ships via `/master`. |
| [QUAL-021](quality.md) | Frontend | **Cross-cutting dedup (composition)** — add a `ShellChromeScope` *component* a page contains (not a page base class): set title/subtitle on init + first-render, restore on dispose — removing the per-page shell-chrome push/restore boilerplate and the BUG-044 re-apply workaround duplicated verbatim in `RequesterTracking`/`RequesterComplete`; marshal the off-render-thread `Shell.SetTitle` calls through `InvokeAsync`. Ships via `/master`. |
| [QUAL-022](quality.md) | Frontend | **Dedup** — extract a shared `TripMap` component for the ~60-line map-overlay engine (rep marker + requester pin + route polyline + on-site collapse + status→chip switch) duplicated between `ActiveJob` and `RequesterTracking`, keeping the two trip views in lock-step. Ships via `/master`. |
| [QUAL-023](quality.md) | Frontend | **DRY (composition / leave-it)** — de-duplicate the four near-identical `InMemory*Store` classes via a contained `Cell<T>` value-holder (composition, not an `abstract` base), keeping the four interfaces separate per ISP-per-capability, and reconcile the `CurrentOffer`/`SetOffer` vs generic `CurrentPayload`/`SetPayload` naming. **Marginal win** — legitimate leave-it candidate; only if the batch is already in `Core/Services`. Ships via `/master` if pursued. |
| [QUAL-024](quality.md) | Frontend | **DRY** — collapse the three byte-for-byte `{Success, Conflict}` result enums (`AcceptOfferResult`/`DeclineOfferResult`/`TakeOverResult`) into one `OperationOutcome`; leave `ReleaseVehicleResult` and the `SubmitServiceRequestResult` closed DU as legitimately distinct. Ships via `/master`. |
| [QUAL-025](quality.md) | Frontend | **DI consistency** — restore Dependency Inversion for the only page that `new`s its ViewModel: introduce an `IJobOfferViewModelFactory` that builds `JobOfferViewModel` from the runtime `JobOfferPayload` + DI-resolved services, registered per host. Ships via `/master`. |
| [QUAL-026](quality.md) | Frontend | **Judgement call (low)** — evaluate consolidating over-segregated ISP pairs whose single consumer uses both halves (`IArrive`+`ICompleteJob` in `ActiveJobViewModel`; `IJobOffer`+`IDeclineOffer` in `JobOfferViewModel`) into `IRepJobActionService`/`IJobOfferResponseService`, or close as won't-do with rationale. Runs counter to the repo's ISP convention — needs a conscious decision. Ships via `/master` if merged. |
| [QUAL-027](quality.md) | Frontend | **Convention** — establish one rule for `.razor.cs` code-behind vs inline `@code` (no rule followed today; `RequesterPending` splits both ways) and migrate components to it. Frontend code + CLAUDE.md via `/master`; any pipeline-doc edit via `/ship-it` (mixed). |
| [QUAL-028](quality.md) | Frontend | **Decision/doc** — record when the MudBlazor-first rule yields to raw `sd-*` CSS: the map/trip screens (`ActiveJob`/`RequesterTracking`/`JobOffer`/`RequesterComplete`) use hand-authored `sd-*` chrome for pixel-matched mockups, contradicting the CLAUDE.md rule. Amend the UI Framework section (or migrate). Frontend via `/master`; CLAUDE.md-only outcome via `/ship-it`. |
| [QUAL-029](quality.md) | Simulator | **Realism** — make the simulator operate reps like a human: consider one offer at a time (no concurrent double-offer per rep), human pacing, and **decline-on-409** (surface the 409 from `IBackendApiClient.AcceptJobOfferAsync` and relinquish so the request re-matches immediately). Removes the machine-speed contention that produced BUG-057/061 at their source. From the BUG-055 retrospective (2026-07-23). Via `/master` (simulator). |
| [QUAL-030](quality.md) | Frontend | **E2E isolation** — give the redirect + requester-tracking Playwright tests a dedicated fleet/dealer so they stop over-contending the shared 7-rep pool; **this is what finally greens BUG-055** (merge the held `fix/BUG-055-requester-redirect-enroute-arrange` branch + confirm `RequesterRedirectTests` across 2–3 fresh boots). From the BUG-055 retrospective (2026-07-23). Via `/master` (frontend E2E). |
...
Plus **61 bugs** ([`bug.md`](bug.md)) — `BUG-001` – `BUG-061`; all resolved except `BUG-049`, `BUG-055`, and `BUG-056` (open). `BUG-061` (a 409-rejected offer left `Pending` until ~60 s expiry instead of being released/re-matched — surfaced by BUG-057's now-correct guard) was **fixed** via `/master` (backend PR #61, merged 2026-07-23: `Expire()`+re-match before the 409). **Retrospective (2026-07-23):** the BUG-055 dig fixed six real defects, but only `BUG-060` (offers silently dropped) and `BUG-058` (a backend background-service race) are broadly real; `BUG-057`/`BUG-061` (and partly `BUG-059`) are machine-speed/concurrency guards a **human client cannot trigger** (a human rep is offered one job at a time). The redirect test's residual redness is a **self-inflicted E2E fleet-contention artifact**, not a product bug — now tracked under [QUAL-029](quality.md) (make the simulator human-realistic) + [QUAL-030](quality.md) (isolate the redirect/tracking E2E fleet), which together close `BUG-055`. `BUG-058` (the sweeper/reconciler race creating duplicate concurrent `Pending` offers) was **fixed** via `/master` (backend PR #60, merged 2026-07-23: `RunAsync` idempotency guard on a live-Pending query, BUG-054 re-offer behaviour preserved). `BUG-057` (accept had no active-job guard, letting a rep clobber its `ActiveRequestId`) was **fixed** via `/master` (backend PR #59, merged 2026-07-23; guard → 409, AC-3 via guard-based sequenced safety with the InMemory same-instant-race documented). `BUG-060` (the simulator typed `JobOfferReceived.etaMinutes` as `int` vs the backend's `double`, silently dropping every fractional-ETA offer — an ADR-0011 wire-contract drift, sibling of BUG-036) was found during BUG-055's live gate (2026-07-22/23) and **fixed** via `/master` (simulator PR #28, merged 2026-07-23). `BUG-055`'s own arrange defect is also fixed (branch `fix/BUG-055-requester-redirect-enroute-arrange`, PR held) and the redirect POST now succeeds reliably; its `RequesterRedirectTests` still can't go green because the requester E2E suite over-contends the finite 7-rep fleet at machine speed (a test-design/simulator-realism artifact), tracked under `QUAL-030` + `QUAL-029` — merge the held branch and confirm AC-2 as part of `QUAL-030`. `BUG-057` (accept had no active-job guard) and `BUG-058` (sweeper/reconciler duplicate offers) are now **fixed** (backend PRs #59, #60, merged 2026-07-23). `BUG-059` (the one-way `EnRoute → Within15Miles` proximity latch that makes a redirect permanently impossible once a rep is within 15 mi) was root-caused during BUG-055's `/master` live gate (2026-07-22, three failed fresh-boot runs proving no frontend-only redirect arrange can hold a rep `EnRoute` against the simulator's Navigate driver) and **fixed** via `/master` (backend PR #58, merged 2026-07-22: bidirectional recompute with 15/17 mi hysteresis, ADR-0012); it unblocks `BUG-055`, now re-pointed as its frontend follow-up. `BUG-055` (Playwright redirect arrange helper matches only `EnRoute`, missing reps already at `Within15Miles`) and `BUG-056` (Playwright fleet-marker click races the 3 s Maps-SDK marker re-render via a stale `IElementHandle`) are frontend test-infrastructure flakes filed 2026-07-21 from `BUG-053`'s completed live gate — neither is offer starvation; both are in frontend E2E test code untouched by the `BUG-053` fix. `BUG-057` (`POST /job-offers/{id}/accept` has no active-job guard, so one rep can hold multiple concurrent jobs) and `BUG-058` (the expiry-sweeper / pending-reconciler race creates duplicate concurrent offers) are the two **backend** related observations recorded in `BUG-054`, filed 2026-07-21 as their own bugs from `BUG-049`'s resumed gate — both open. `BUG-053` and `BUG-054` are now **resolved** (simulator hub self-heal; backend PR #57 — expired offers no longer skip-list a rep). `BUG-053` (simulator rep RepHub connections die silently and stay dead — no hub logging, `Closed` unobserved, bounded reconnect then permanent silent death — leaving deaf-but-`Available` reps whose offers all expire) and `BUG-054` (expired job offers permanently poison a request's skip list, making a fully-cycled request unmatchable forever) were both root-caused live during `BUG-049`'s **resumed** live gate (2026-07-20) with a fresh-listener probe that exonerated the backend send path; together they are the remaining root cause of the `BUG-049` pending-starvation flake — see Phase 11. `BUG-052` (the simulator rep-claim livelock — every vehicle-less rep stampedes `FirstOrDefault` of the take-over listing `/vehicles/available`, 409s forever with no skip/backoff, starving vehicles 5–8 and the whole E2E fleet non-deterministically) was found during `BUG-049`'s `/master` live gate — its added fleet-state diagnostic is what exposed it — and was fixed via `/master` (simulator PR #26), with the fix live-confirmed during the resumed gate (all 8 vehicles claimed on a fresh boot); see Phase 11. `BUG-051` (unclaimed vehicles hidden by the fleet snapshot but grey after a position event — a backend snapshot/event vocabulary mismatch) was FE-003's review advisory, live-confirmed and filed from its Desktop gate, and fixed via `/master` (backend PR #56: snapshot aligned to the broadcast vocabulary, unclaimed → `"Unassigned"`/grey) — see Phase 11. `BUG-041`–`BUG-048` (ServiceRep idle/take-over state and concurrency, requester-tracking app-bar fidelity, and Appium E2E timing/under-load flake hardening) were all fixed via `/master` during the Phase 9–10 live-verification and QUAL-011 re-verification work — see their struck rows in Phase 9. `BUG-049` (the Playwright `RequesterRedirectTests` suite-mode arrange flake — three observed failure modes, control-experiment-proven to pre-exist FE-003) is **open**, filed from FE-003's live gate. `BUG-050` (valid-token cold start rendered a permanently blank `/` on every host, hidden by a negative-only masking launch test) was exposed by FE-003's Desktop live gate and fixed in frontend PR #76 — see Phase 11. `BUG-003`–`BUG-015` were central-repo doc/pipeline fixes (shipped via `/ship-it`). `BUG-024`–`BUG-026` were frontend E2E failures found via `test-e2e.sh`. `BUG-027`–`BUG-031` were found via `test-appium.sh` (backend idle-vehicle semantics, frontend REST + SignalR auth headers, startup error, and Appium harness defects); `BUG-032` then made the suite's 4 job-offer tests pass (backend-only run + a fleet-positioning/request precondition) and `[Ignore]`d the JWT-expiry test, taking the Appium suite to **8/9 passing, 1 skipped**. `BUG-033` (rep take-over/idle mockup fidelity, PR #36), `BUG-034` (idle view showed a hardcoded vehicle, PR #37), and `BUG-035` (surface the vehicle model in the DTO and render "<reg> · <model>", backend PR #47 + frontend PR #38) were fixed via `/master`. `BUG-036` (job-offer tier badge invisible + app-bar/card fidelity — a `JobOfferReceived` wire-contract mismatch defaulting `Tier` to `None`) was fixed via `/master` (frontend PR #45), live-verified by the Appium `JobOfferTests` (4/4). `BUG-037` (frontend ignored the `JobOfferExpired` RepHub event) was fixed via `/master` (frontend PR #47). `BUG-038` (idle-screen unhandled-error banner when RepHub can't connect at mount) was fixed via `/master` (frontend PR #48). `BUG-039` (active-job screen mockup-fidelity gaps — app-bar title, ETA-card distance + placement, En Route chip style) was fixed via `/master`: the root cause was the screen reading the impoverished `service-requests/my-active` endpoint instead of the purpose-built `rep/active-job-state` (which carries real rep position, ETA, and — after this fix — distance + tier), so the backend surfaced `DistanceMiles` + `Tier` (backend PR #49) and the frontend repointed the endpoint and closed all four gaps (frontend PR #49), live-verified end-to-end. `BUG-040` (the `JobOfferExpired` Appium E2E test could never pass — a 15 s throwing `WaitForSignalR` vs a ~60 s server offer-expiry, so it aborted on its first lap) was found running `test-appium.sh` after BUG-039 and fixed via `/master` across three repos: backend made the offer-expiry configurable (default 60 s; backend PR #50), `test-appium.sh` shortens it to 15 s for the E2E run via an env override (central PR #165), and the test uses a non-throwing `TryWaitForSignalR` so its loop governs the wait (frontend PR #50) — live-verified, Appium `JobOfferTests` 6/6.

Plus **30 engineering-quality stories** ([`quality.md`](quality.md)) — `QUAL-001` – `QUAL-030`; `QUAL-001`–`QUAL-013` complete, `QUAL-014`–`QUAL-030` open (`QUAL-029` simulator human-realism [Simulator] and `QUAL-030` redirect/tracking E2E fleet isolation [Frontend] filed 2026-07-23 from the BUG-055 retrospective — QUAL-030 closes BUG-055) (all Frontend, filed 2026-07-21 from a read-only design investigation of the frontend shared code — `Core` + `UI` — ordered by importance: `QUAL-014` the highest-leverage reliability defect down to convention/decision items; `QUAL-014`/`QUAL-015` are defect-flavoured and could be reclassified as `BUG-`). `QUAL-011` consolidated the shared design-system tokens into a global stylesheet loaded by all three hosts, killing the recurring scoped-CSS gap. `QUAL-012` (frontend PR #77) routed `HubConnection` internal logging through the host logger in the Rep/Requester hub clients — parity with FE-003's vehicle-hub fix, so client-side SignalR transport failures are never silent. `QUAL-013` (backend PR #55) schematized all 24 REST success responses in the committed OpenAPI contract (previously 0/24 — request bodies only), so the QUAL-006 sync-check now guards the shapes consumers actually bind; both were filed from FE-003's live-gate forensics. They were filed from a retrospective on why integration defects clustered in the frontend (many untestable boundaries + a live net that arrived late) while the backend and simulator stayed clean (one peer, `smoke.sh` from day one). They harden the frontend integration loop: run the live net early (`QUAL-005` — done: `smoke-web.sh` + the `story-ai-reviewer`/`master` live-green rule), enforce wire-contract integrity across repos (`QUAL-006` — done: ADR-0011 + committed backend OpenAPI contract with a sync-check, fail-loud enum deserialization + captured-payload contract tests in the frontend and simulator, and the `/test-quality` captured-payload rule), test the real composition root not isolated components (`QUAL-007` — done: the `/test-quality` Frontend composition-root rule + `story-ai-reviewer` Checks 2/3b referencing it, and a frontend handler-chain composition test that exercises the real `SessionExpiry → AuthToken → network` pipeline), and treat each client runtime as its own integration target (`QUAL-008` — done: a per-runtime smoke set off one entry point [`smoke-web.sh` browser-CORS, `smoke-mobile.sh` WKWebView data-testid reachability, `smoke.sh` headless], the backend CORS regression test, and a host-parity rule in the frontend CLAUDE.md enforced by `story-ai-reviewer` Check 11). `QUAL-009` live-verified the FE-023/BE-028/SIM-009 go-off-duty / heartbeat-timeout chain end-to-end (each layer had been proven only in isolation). `QUAL-010` resolved the backlog from the QUAL-007 `/audit-skills` run — done: `/master` now documents the `QUAL-`/unmapped-prefix → `/ship-it` redirect (no longer reliant on operator memory), `tdd-cycle` tracks the current bUnit v2 API, the audit skills gain a stale-API/version red flag, and the `test-quality`/`solid-principles` consistency nits are closed.
