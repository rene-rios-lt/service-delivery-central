...
| [**BUG-018**](bug.md) | Central | **Bug (reframed)** — the reported `Within15Miles` "navigation stall" was a misdiagnosis: live re-diagnosis (2026-06-19) confirmed the automated cycle completes end-to-end with monotonic convergence. Real issue: `scripts/local/smoke.sh` can falsely time out — it places the requester at a stale idle-vehicle position (sparse waypoints → long legs) and watches only ~6 min. Fix the smoke harness; ships via `/ship-it`, not `/master`. Smoke fix drafted but **held** pending BUG-019. See bug.md. |
| [**BUG-019**](bug.md) | Simulator | **Bug (unconfirmed)** — intermittently the sim never accepts offers (cold-start: all offers expire, zero `/job-offers` calls) or drops an `OnSite` job back to `Pending` mid-dwell. Suspected per-rep RepHub / heartbeat instability (cf. BE-023 / BE-028). Needs a clean cold-start repro to rule out an environmental cause before committing to a fix. Blocks landing the BUG-018 smoke fix. See bug.md. |
