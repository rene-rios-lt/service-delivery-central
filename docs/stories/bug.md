...
- **Found:** BUG-049 `/master` live gate, 2026-07-20 (`test-playwright.sh`-equivalent full-suite fresh-boot runs against the BUG-049 worktree). Across 3 consecutive full-suite fresh boots plus an isolation run, the simulator's claim attempts piled onto the front of the list and never reached the tail: full-suite run — vehicle 1 = **464** claim attempts, vehicle 2 = 344, vehicle 3 = 62, **vehicles 4–8 = 0**; isolation run — vehicle 1 = **380** attempts, **vehicles 2–8 = 0** (only vehicle 1 ever claimed; dispatcher fleet snapshot showed 1 `Available`/`Within15Miles` rep and 7 `Offline` slots with no rep). Verified by a read-only trace of the simulator + backend source.

**Summary**
Each simulated rep with no vehicle picks a target by taking the **first** entry of `GET /vehicles/available` (`FleetClaimCoordinator.cs:58`, plain `FirstOrDefault`) — there is no ordering, no jitter, and no rep-index-based assignment anywhere in the path, so **every** vehicle-less rep converges on the *same* front-of-list vehicle. The implicit assumption ("a claimed vehicle leaves the available list, so the next rep advances to the next one") is **false**: the backend keeps a claimed vehicle in `/vehicles/available` for as long as its claiming rep is idle (`Available` with no active job — the take-over listing, `GetAvailableVehiclesQueryHandler.cs:45-55` + `RepStateRecord.IsOnActiveJob()`), and a fresh claim sets the rep to `Available`. So claimed-but-idle vehicles sit permanently at the front of the list. On the resulting 409 `Conflict`, the rep does **not** back off or skip the conflicted id — it re-fetches on the next 3 s reconciler tick and re-selects the same `FirstOrDefault`, in lock-step with every other vehicle-less rep. A genuinely-unclaimed vehicle (5–8) is only ever exposed at the front if the holders of every vehicle ahead of it are *simultaneously* on active jobs at the instant a rebalance runs — an alignment that never occurred in a 4-minute run, so vehicles 5–8 received **zero** claim attempts and the fleet stayed starved. The whole E2E suite draws its `Available` qualified reps from this fleet, so the starvation surfaces downstream as "no HydraulicTool rep accepted the request" (the BUG-049 pending-starvation mode) and "no fleet markers rendered".

**Expected**
On a fresh boot the simulator claims all 8 seed vehicles promptly and deterministically — each rep converges on a distinct unclaimed vehicle — so the full fleet is `Available` and claimable within a bounded, boot-independent window, giving the E2E suite a healthy fleet regardless of run composition or timing.

**Actual**
Reps stampede the front of the take-over listing and receive 409 after 409 on already-claimed vehicles; only the first 1–4 vehicles are ever claimed and vehicles 5–8 can go an entire run with zero claim attempts. Which vehicles get claimed, and how fast, is a per-boot dice roll on transient active-job alignments — non-deterministic fleet starvation.

**Proposed fix (via `/master`)**
Make claim selection collision-free and self-healing on the simulator side (the backend take-over listing is intentional and should not change). Candidate directions, in preference order: (1) **claim-and-remove-locally / distinct targets** — track ids this simulator instance has already claimed (a `YieldedRepRegistry`-style local set) and skip them, and/or assign each rep a distinct starting offset (e.g. rep index modulo candidate count) so reps do not all pick index 0; (2) **advance past 409s with backoff** — on `Conflict`, drop that id from the candidate set for this pass and try the next unclaimed one instead of re-selecting `FirstOrDefault`, with jittered backoff so the herd disperses across ticks rather than re-synchronising every 3 s; (3) optionally, have the simulator distinguish *unclaimed* from *claimed-but-idle-and-takeable* — it only wants unclaimed vehicles at startup — either via a query parameter the backend already supports or by filtering on the claiming-rep field the listing carries. Do **not** simply widen timeouts or add sleeps — the race is structural (undifferentiated `FirstOrDefault` + no-skip retry vs a listing that never drops claimed-idle entries), so more time only lowers the frequency. Verification gate mirrors BUG-048/049: repeated fresh-boot full-suite runs (2–3× consecutive) where the dispatcher fleet snapshot shows all 8 vehicles claimed within a bounded window, not a single lucky boot.

**Acceptance criteria (bug resolved when):**
- On a fresh boot, all 8 seed vehicles are claimed within a bounded, boot-independent window, with each rep converging on a distinct vehicle (no vehicle receives an unbounded stampede of 409s; vehicles 5–8 are claimed as reliably as 1–4), verified across 2–3 consecutive fresh boots.
...
- **Status:** Open — **arrange + redirect FIXED and verified; test not yet green, now BLOCKED on [BUG-061] (last blocker).** The frontend fix is on branch `fix/BUG-055-requester-redirect-enroute-arrange` (pushed, **PR held**): `WaitForEnRouteRepWithFarPinAsync` re-pins the assigned vehicle each poll iteration so [BUG-059]'s bidirectional recompute returns it to `EnRoute`, and `TriggerRedirectAsync` was reordered (commit 0b71812) to do all slow prep up front then a tight far-pin→redirect retry, closing the `RepNotEnRoute` (422) sim-override race. Live gate 2026-07-23: the arrange **and** `POST /dispatcher/redirect` now succeed reliably. The remaining failure is the post-redirect **displaced request not re-accepted within 45 s** — root-caused to [BUG-061]: under the shared finite fleet the losing rep's offer is 409-rejected (by [BUG-057]'s now-correct guard) but left `Pending` until ~60 s expiry instead of being released/re-matched. Prerequisites already merged: [BUG-059], [BUG-060], [BUG-057], [BUG-058]. **Merge this branch and verify AC-2 green once [BUG-061] lands.**
...
**Proposed fix (via `/master`) — CORRECTED 2026-07-21 (the original one-line proposal is UNSAFE for the redirect path; analysis below)**

❌ *Original proposal — do NOT implement for redirect:* "broaden the helper's predicate to accept `EnRoute` **or** `Within15Miles`." For the **redirect** path this is unsafe. `POST /dispatcher/redirect` rejects any non-`EnRoute` rep (`RedirectRepCommandHandler.cs:101` — *"only an EnRoute rep can be redirected"*), and `Within15Miles` is a **one-way latch**: `UpdateVehiclePositionCommandHandler.cs:52` recomputes proximity **only while the rep is `EnRoute`**, so once a rep is `Within15Miles`, moving its truck back out past the 15-mi threshold does **not** return it to `EnRoute`. Broadening the redirect accept-set would therefore hand the redirect a rep the handler then rejects — converting a 30 s-timeout flake into a **deterministic `RedirectNotAllowed` failure**. (The same broadening is safe for the **completion** path only because its very next call, `POST /rep/arrive`, itself requires `Within15Miles` — which is exactly what PR #78 did there.)

✅ *Real fix for the redirect flake — pick one, via `/master`:*
- **(A) Frontend-only, race-window reduction.** Identify the assigned rep by active-request coords accepting `EnRoute` **or** `Within15Miles`, but the instant it is found (or as soon as it is `EnRoute`) pin its vehicle **far** from the tracked request and re-pin each poll so an `EnRoute` rep never crosses into `Within15Miles` before the redirect. Caveat: if the rep is *already* `Within15Miles` when found, there is no frontend-only recovery (the latch is server-side) — this shrinks the race window, it does not fully close it.
- **(B) Backend product decision, full fix.** Make the proximity transition **bidirectional** — recompute `EnRoute ⇄ Within15Miles` on every position update regardless of current state, so a truck that moves back out past 15 mi returns to `EnRoute`. This closes the race completely and is arguably more correct, but it is a backend behaviour change with redirect-eligibility implications and needs a product decision (does moving away legitimately re-open a rep to redirect?).

Verify the chosen fix with `RequesterRedirectTests` green across 2–3 consecutive fresh-boot `test-playwright.sh` runs.
...
- **Status:** Fixed — backend PR #59 (merged 2026-07-23). `AcceptJobOfferCommandHandler` now rejects an accept when `repState.IsOnActiveJob()` (new `RepAlreadyOnActiveJobException` → 409) before any mutation, so an on-job rep can no longer clobber its `ActiveRequestId`. AC-3 handled via guard-based sequenced safety (Option B); the true same-instant double-read race under the InMemory EF provider is documented as a POC limitation (row-version/SQL follow-up). Contributes to unblocking BUG-055 (still needs BUG-058).
...
- **Found:** `BUG-049` resumed `/master` live gate, 2026-07-20. Rep One accepted **two** offers within the same second while a third job was already `InProgress`, ending with three concurrent jobs. **Reproduced again 2026-07-23** during the `BUG-055` live gate: rep5 received two `Gold` offers for two different requests and POSTed `accept` for **both** — this (with `BUG-058`) is what stops the redirect test's displaced request from being cleanly re-accepted, so `BUG-055`'s test cannot go green until `BUG-057`/`BUG-058` are fixed.
...
- **Status:** Fixed — backend PR #60 (merged 2026-07-23). `MatchingService.RunAsync` is now idempotent: a new `GetLivePendingOfferForRequestAsync` query + guard short-circuit when a live (`Pending`, unexpired) offer already exists, so the sweeper/reconciler race can no longer create duplicates. Keyed strictly on live Pending, so BUG-054's declined/expired re-offer behaviour is preserved. AC-2 true-concurrency is a documented InMemory POC boundary (DB unique-index follow-up).
...
- **Status:** Fixed — backend PR #58 (merged 2026-07-22). Bidirectional `EnRoute ⇄ Within15Miles` recompute with hysteresis (enter < 15 mi / exit ≥ 17 mi); product decision recorded in ADR-0012; state-machines doc updated. Unblocks BUG-055 (its frontend redirect arrange is now viable).
...
## BUG-060 — Simulator types `JobOfferReceived.etaMinutes` as `int` while the backend sends a `double`, so every offer with a fractional ETA fails to deserialize and is silently dropped — the rep never accepts

- **Status:** Fixed — simulator PR #28 (merged 2026-07-23). `JobOfferPayload.EtaMinutes` retyped `int → double` + a captured-payload round-trip contract test (ADR-0011). Live-confirmed active during the BUG-055 gate: the `$.etaMinutes` bind failures are gone and offers now deserialize and get accepted.
- **Severity:** High (product reliability / silent data loss — a wire-contract mismatch, ADR-0011 class. The simulator's SignalR `JobOfferReceived` handler cannot bind any payload whose `etaMinutes` is non-integral: `System.Text.Json` throws `The JSON value could not be converted to … JobOfferPayload. Path: $.etaMinutes`, the whole hub invocation fails to bind, and `JobOfferDecisionEngine.HandleOfferAsync` is never invoked. The rep — though `Available` and healthy — silently ignores the offer, which then expires. Only offers with a whole-number ETA (e.g. a rep sitting exactly at the requester → `EtaMinutes(0)=0`) bind and get accepted, which is why the defect hid behind the E2E tests that pin the fleet at the requester. Any real, non-zero distance produces a fractional ETA and the offer is lost.)
- **Repo / Area:** **Simulator** — `src/ServiceDelivery.Simulator/Models/JobOfferPayload.cs` (`int EtaMinutes` — line 12). The backend's authoritative contract is `double`: `Application/Common/Interfaces/Payloads/JobOfferReceivedPayload.cs:12` (`double EtaMinutes`), populated from `HaversineCalculator.EtaMinutes(double) : double` (unrounded). The frontend consumes it as `double` (correct); the simulator is the sole drifted consumer.
- **Related stories:** `BUG-036` / ADR-0011 (the wire-contract-integrity decision and the captured-payload deserialization-test convention this bug violates — a consumer's hand-mirrored model silently drifted from the backend shape and `System.Text.Json` failed at the boundary). `BUG-055` (BLOCKED by this: the redirect test's post-redirect displaced-request re-accept never fires because the re-match offer carries a fractional ETA the simulator drops). `BUG-053`/`BUG-054` (prior offer-loss root causes on the same path — deaf hubs / skip-list; this is a third, independent offer-loss mechanism). `BUG-057`/`BUG-058` (other open backend offer-path concurrency bugs surfaced by the same gate).
- **Found:** BUG-055 `/master` live gate, 2026-07-22/23. With BUG-059 merged, the redirect test progressed past the arrange and the redirect POST (both now succeed) but failed deterministically (2/2 fresh boots) at `WaitForRedirectBannerWithFleetRepositioningAsync` — "displaced request never re-accepted". The simulator log showed repeated `Failed to bind arguments received in invocation '(null)' of 'JobOfferReceived' … Path: $.etaMinutes` interleaved with the offers that DID bind (whole-number ETA), pinpointing the `int`/`double` drift.

**Summary**
The backend broadcasts `JobOfferReceived` with `etaMinutes` as a `double` (unrounded Haversine ETA). The simulator's `JobOfferPayload` record declares `int EtaMinutes`. `System.Text.Json` (Web defaults) will bind a whole-number JSON number to `int` but throws on a fractional one, and because the field is a positional constructor argument the whole payload fails to deserialize — the SignalR handler binding fails and the decision engine is never called. The offer is silently lost; the rep never accepts or declines; the request eventually expires or starves.

**Expected**
The simulator deserializes every valid `JobOfferReceived` payload the backend sends — including fractional `etaMinutes` — and responds (accept/decline) per its decision policy.

**Actual**
Offers with a fractional `etaMinutes` throw during hub-argument binding and are dropped silently; only whole-number-ETA offers are processed.

**Proposed fix (via `/master`)**
Change `JobOfferPayload.EtaMinutes` from `int` to `double` to match the backend contract (the frontend already uses `double`). If the simulator needs a whole number for logging/decisions, round at the point of use, not at the wire boundary. Per ADR-0011, back the fix with a **captured-payload deserialization test**: round-trip a real `JobOfferReceived` JSON string (with a **fractional** `etaMinutes`, e.g. `12.5`, and distinct values per field) through the same `System.Text.Json` path the SignalR client uses, asserting it binds. Audit the simulator's other `JobOfferReceived`/hub payload fields for the same numeric drift while here.

**Acceptance criteria (bug resolved when):**
- A `JobOfferReceived` payload with a fractional `etaMinutes` deserializes successfully in the simulator and reaches `JobOfferDecisionEngine.HandleOfferAsync`.
- A captured real-payload round-trip test (fractional `etaMinutes`, distinct per-field values) guards the contract and fails against the old `int` model.
- Whole-number ETAs still bind (no regression).
- Live-verified: with `AutoDeclineRatePercent=0`, offers with real (fractional-ETA) distances are accepted — and `BUG-055`'s `RequesterRedirectTests` reaches the redirect banner (displaced request re-accepted) across 2–3 consecutive fresh-boot `test-playwright.sh` runs.

---

## BUG-061 — A job offer whose rep then accepts a different job (accept rejected 409 by the BUG-057 guard) is left `Pending` until it expires (~60 s) instead of being released and re-matched, so a contended request stalls

- **Status:** Open
- **Severity:** Medium (matching throughput / latency under contention — surfaced *because* BUG-057's accept-guard now works. No data corruption; a request just waits up to a full offer-expiry (~60 s) for reassignment when its offered rep took a different job. Non-deterministic; worse under a small/contended qualified pool.)
- **Repo / Area:** **Backend** — `Application/Features/JobOffers/Commands/AcceptJobOfferCommandHandler.cs` + `MatchingService` / the offer lifecycle. (Complementary **Simulator** mitigation possible — see below — but the robust product fix is backend, so route there.)
- **Related stories:** `BUG-057` (its now-correct 409 guard is what exposes this — before it, the second accept clobbered instead of being rejected), `BUG-058` (dedups offers for the *same* request; this is the *different-requests-same-rep* case it does not cover), `BUG-055` (the redirect test whose post-redirect displaced request stalls on exactly this — the last thing keeping its `RequesterRedirectTests` red after 059/060/057/058 + the arrange fix all landed), `BUG-054`/`ExpiredJobOfferSweeper`/`PendingRequestReconciler` (the ~60 s expiry-then-re-match path that currently the only thing eventually rescues the stalled request), `docs/business-rules.md` (matching / one-rep-one-job).
- **Found:** `BUG-055` close-out live gate, 2026-07-23. Sim log: rep4 received two `Gold` offers for two different requests, accepted the first, and its second accept returned **409 Conflict** (`POST /job-offers/{id}/accept returned Conflict for rep …4`); the second request's offer then sat `Pending` with no re-match, and `RequesterRedirectTests` timed out at 45 s waiting for the displaced request's banner. Same shape observed for rep8.

**Summary**
Matching offers each pending request to the single nearest eligible rep. Under a contended pool, the same rep is the nearest for more than one pending request and receives concurrent offers for different requests. It accepts one; with BUG-057's guard, every other accept is correctly rejected (409). But nothing then **releases** those now-doomed offers or **re-matches** their requests — the offers remain `Pending` until they expire (~60 s), at which point the `ExpiredJobOfferSweeper` finally re-matches. A request can therefore stall for close to a full offer-expiry even though other qualified reps are idle.

**Expected**
When a rep cannot accept an offer because it is already on an active job (or the offer is otherwise un-acceptable), that offer is released promptly and its request is re-matched to the next eligible rep — not left pending until expiry. A contended request is assigned as soon as another qualified rep is available.

**Actual**
The rejected offer stays `Pending` until ~60 s expiry; the request is only rescued by the periodic sweeper/reconciler. Under the E2E redirect scenario this exceeds the 45 s assertion window and the displaced request appears never re-accepted.

**Proposed fix (via `/master`)** — decide the seam:
- **(Backend, preferred) Release-and-re-match on rejected accept:** when `AcceptJobOfferCommandHandler` rejects because the rep is on an active job, transition that offer out of `Pending` (e.g. a `Superseded`/`Released` status or straight to expired) and immediately `RunAsync` the request so it re-matches to the next eligible rep. Consider not offering the same request to a rep that already holds a live offer for a *different* request.
- **(Simulator, complementary) Decline-on-409:** `JobOfferDecisionEngine` should, on a 409 from accept, `POST /decline` (or otherwise free the offer) so matching's existing decline→re-match path fires immediately instead of waiting for expiry.
- Tests: a rep offered two different requests accepts one and the other request is re-matched to a different idle rep without waiting for expiry; the single-rep no-other-candidate case still falls back to expiry cleanly; BUG-054 re-offer semantics preserved.

**Acceptance criteria (bug resolved when):**
- When a rep's accept is rejected (409, already on an active job), the offer it could not take is released and its request is re-matched to the next eligible idle rep without waiting for the ~60 s expiry.
