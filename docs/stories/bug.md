...
- **Status:** Open — **arrange defect FIXED and verified; test not yet green, now BLOCKED on [BUG-057]/[BUG-058].** [BUG-059] (merged) unblocked the arrange, and the frontend fix is implemented on branch `fix/BUG-055-requester-redirect-enroute-arrange` (commit dd2c404, pushed, **PR held**): `WaitForEnRouteRepWithFarPinAsync` re-pins the assigned vehicle every poll iteration so BUG-059's bidirectional recompute returns it to `EnRoute` before the redirect. Live gate (2026-07-23) confirmed the arrange + `POST /dispatcher/redirect` now **succeed** (the 3× pre-BUG-059 failure is gone). The test still fails one step later — the post-redirect **displaced request is never re-accepted** — which the live run traced to [BUG-057] (a rep accepting concurrent jobs — observed live: rep5 accepted two offers) / [BUG-058] (duplicate concurrent offers), tripped by the redirect scenario's concurrent Gold-target + displaced re-match. **Merge this branch and verify AC-2 green once BUG-057/BUG-058 land.** (Also fixed a third blocker en route: [BUG-060], the simulator `etaMinutes` int/double drop.)
