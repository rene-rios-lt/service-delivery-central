...
- **Status:** Fixed — backend PR #61 (merged 2026-07-23). `AcceptJobOfferCommandHandler` now `Expire()`s the rejected offer, persists it, and re-runs matching for the request before throwing the 409, so a contended request is reassigned to the next idle rep without waiting for expiry (`Expire()` over `Decline()` to preserve BUG-054 re-eligibility; backend-only). **Note (retrospective):** this and BUG-057 are machine-speed/concurrency guards a human client cannot trigger (a human rep is offered one job at a time); the live gate showed the redirect test's residual failure is shared-fleet **exhaustion** (a self-inflicted E2E contention artifact), not this bug — so AC-4 (BUG-055 live gate) is deferred to the simulator-realism + fleet-isolation work ([QUAL-029]/[QUAL-030]).
