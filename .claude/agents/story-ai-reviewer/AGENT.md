...
**Frontend composition-root level** *(applies to `FE-` stories and frontend `BUG-` stories — skip for other repos):* when an AC depends on the DI pipeline / composition root, the `HttpClient` `DelegatingHandler` chain, or the Blazor render/navigation lifecycle (`OnInitialized` vs `OnParametersSet`, parameter-diffing, the Router reusing a layout across navigations), a ViewModel or bUnit test in isolation is **not** the sufficient level — that behaviour must be exercised through the **real composition** (rendered from `/login` then navigated; the real handler chain assembled in host-registration order). See the test-quality skill's **Frontend composition-root rule (QUAL-007)**. A layout test rendered directly on a non-`/login` route, or a handler proven only in isolation when the defect is its position in the pipeline, is the wrong level for that AC — flag it here; the masking judgment itself is Check 3b.
...
When an AC is E2E-only, determine the platform(s) the persona is served on from the story text and the [Persona Platform Support](../docs/architecture/system-overview.md) table, then apply:

- **Web platform:** check whether `tests/ServiceDelivery.Client.E2E/` exists (`Glob("tests/ServiceDelivery.Client.E2E/**/*.csproj")`).
  - Project exists + no Playwright scenario for that AC in the diff → **blocking** finding: `[Missing E2E test — Playwright]`
  - Project absent → **advisory** only: `[E2E project not set up — run QUAL-003 first]`
- **Desktop platform:** check whether `tests/ServiceDelivery.Client.Appium.Mac/` exists (`Glob("tests/ServiceDelivery.Client.Appium.Mac/**/*.csproj")`).
  - Project exists + no Appium Mac2 scenario for that AC in the diff → **blocking** finding: `[Missing E2E test — Appium Mac2 (Desktop)]`
  - Project absent → **advisory** only: `[Desktop E2E project not set up]`
...
**Web/Desktop parity (Dispatcher).** A persona served on **both** Web and Desktop (the Dispatcher — same shared `Client.UI`/`Client.Core` screen) requires **both** a Playwright scenario **and** a mirrored Appium Mac2 (Desktop) scenario for each E2E-only AC — apply the Web *and* the Desktop bullets above, not just one. Web-green ≠ WebView-green (Desktop is MAUI `BlazorWebView`/WKWebView, a different runtime — the BUG-020 → BUG-022 / BUG-031 lesson extended to E2E), so a Playwright scenario present without its Desktop mirror (when `Client.Appium.Mac` exists) is a **blocking** `[Missing E2E test — Appium Mac2 (Desktop)]`. The one legitimate exception is an AC that is intrinsically browser-only (responsive reflow at narrower *web* widths, e.g. FE-004 AC-5) — accept a stated web-only exemption in `03-implementation.md` and do not require a Desktop mirror for it. This mirrors the frontend `CLAUDE.md` **Dispatcher Web/Desktop E2E parity** rule.
...
- If there is no such evidence — the scenario was **authored but never run** — record a **blocking** finding `[E2E authored but not run live]` naming the AC and the scenario, with the fix: *run it green against a live system (`../scripts/local/start.sh` + the suite, or `test-playwright.sh` / `test-appium.sh` / `test-appium-mac.sh`) and record the result before this story is declared done.* For a Dispatcher (Web + Desktop) AC this applies to **both** the Playwright and the Appium Mac2 (Desktop) scenarios — each must have its own live-green evidence (per the Web/Desktop parity rule above); a green web run does not clear the Desktop mirror. Do **not** clear this finding on the strength of the offline suite alone.
...
- **Frontend composition-root bypass** *(frontend stories — apply the test-quality skill's Frontend composition-root rule, QUAL-007)* — a test exercises lifecycle-, handler-chain-, or DI-dependent behaviour without the real composition, so a defect in the integrated app stays green: a layout/shell test rendered directly on a non-`/login` route instead of rendered-then-navigated (BUG-025/026/029 — the `OnInitialized`-vs-`OnParametersSet` / Router-reuse / parameter-diffing path), or a `DelegatingHandler` asserted in isolation with a stubbed inner handler when the defect is its position/interaction in the chain (BUG-024 session-expiry firing on the login 401; BUG-028 the composed pipeline attaching no bearer token). Fix guidance: render from `/login` and navigate to the destination route (exercises `OnParametersSetAsync` and the Router's layout reuse), or assemble the real handler chain in host-registration order (`SessionExpiryHttpHandler → AuthTokenHttpHandler → HttpClientHandler`) and assert the cross-handler interaction — a 401 from `/auth/login` does not throw, an authenticated request carries the bearer header.
