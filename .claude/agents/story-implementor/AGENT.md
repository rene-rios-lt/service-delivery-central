...
**Appium (mobile stories):** if `tests/ServiceDelivery.Client.Appium/` exists and the story's platform includes Mobile, write the Appium test file named in the plan **after** the AC bUnit tests are green (the Appium suite is the live-system complement, not a substitute for the bUnit cycle). Locate elements with `MobileBy.AccessibilityId("<identifier>")` / `By.Id("<identifier>")` — add the matching `id="<identifier>"` attribute to the interactive Razor element alongside its existing `data-testid` (MAUI Blazor on iOS surfaces DOM `id` into the XCUITest accessibility tree). For SignalR-driven screen transitions (e.g. the job offer appearing), set the implicit wait to `TimeSpan.FromSeconds(15)` or use an explicit `WebDriverWait` poll with a ≥15 s budget. Do **not** execute Appium tests during the pipeline — they require a booted iOS simulator, the installed app, and a running Appium server. State in the implementation report: "Appium tests written; execute via `test-appium.sh` against a live system with a booted iOS simulator."
