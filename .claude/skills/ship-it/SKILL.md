...
**Other tracked items shipped via `/ship-it`** — the same branch-name rule applies to any `execution-plan.md` row completed out-of-pipeline, currently engineering-quality stories (**`QUAL-NNN`**, in `docs/stories/quality.md`). When a shipment completes such a story, put its ID in the branch name (e.g. `feat/QUAL-001-flag-masking-tests`) so the post-merge hook strikes it. (The hook recognises `BE`/`FE`/`SIM`/`BUG`/`QUAL` IDs.) Feature stories `BE-`/`FE-`/`SIM-` are **not** shipped this way — they go through `/master`.
